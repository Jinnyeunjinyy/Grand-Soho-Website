...
- HeroCanvas: Hero 섹션 3D 와이어프레임 오버레이. R3F 투명 캔버스, Float + 느린 회전, pointerEvents none (`components/media/HeroCanvas.jsx`)
...
- LogoAssembly3D: SOHO 로고 10개 조각이 3D 메시(torus/cylinder/box)로 항상 같은 로고 배치로 수렴하는 R3F 씬. useFrame 스프링, ContactShadows, 매 실행 동일한 결과 (`components/motion/LogoAssembly3D.jsx`)
...
- HeroSection: Grand Soho 홈 히어로 섹션. 배경 이미지 + HeroCanvas 3D 오버레이 + framer-motion 텍스트 입장 애니메이션. is3DEnabled prop으로 3D 토글 (`components/templates/home/HeroSection.jsx`)
