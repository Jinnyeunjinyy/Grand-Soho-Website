...
- LogoAssembly3D: SOHO 로고 10개 조각이 3D 메시(torus/cylinder/box)로 항상 같은 로고 배치로 수렴하는 R3F 씬. useFrame 스프링, ContactShadows, 매 실행 동일한 결과. isTransparent로 투명 배경 오버레이 모드 지원 (`components/motion/LogoAssembly3D.jsx`)
...
- HeroSection: Grand Soho 홈 히어로 섹션. 배경 이미지 + 3D 투명 오버레이 + framer-motion 텍스트 입장 애니메이션. is3DEnabled로 3D 토글, isAssemblyOnly로 LogoPhysics 대신 LogoAssembly3D(glass) 전체 화면 오버레이 사용 (`components/templates/home/HeroSection.jsx`)
