...
- ContactPage: /contact 문의 페이지. 이름·연락처·유형·내용 폼 + mailto 제출(임시, EmailJS 발급 후 복원 예정) + 인라인 피드백 (`components/templates/contact/ContactPage.jsx`)
- NoticePage: /notice 공지사항 목록 페이지. 카테고리 탭 + 고정공지는 카드 형식으로 상단에, 일반 공지는 구분선 목록으로 하단에 표시 (`components/templates/notice/NoticePage.jsx`)
